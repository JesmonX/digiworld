...
export default function App() {
  ...
  return <PluginPage className="markpad" toolbar={<PageToolbar actions={<Button variant="primary" disabled={!loaded} onClick={add}><Plus size={16}/>{t('写一条','New note')}</Button>}><div><h1>{t('随笔','Notes')}</h1><span className="markpad-muted">{t('随手记下，慢慢回看。','Capture a thought. Return to it later.')}</span></div></PageToolbar>}>
    ...
    {!loaded ? <Status>{t('正在载入随笔…','Loading notes…')}</Status> : <SplitPane aside={<div className="markpad-sidebar">
      ...
      <div className="markpad-footer"><span className="markpad-muted">{plain(current.content).length} / 50000 {t('字符','characters')}</span><Button disabled={saving || !dirty.current.size || remove} onClick={() => void flush()}>{saving ? t('保存中…','Saving…') : t('保存','Save')}</Button></div></> : <EmptyState icon={<Pencil/>} title={t('留一点空间给想法','Make room for a thought')} description={t('选择一条记录回顾，或开始新的随笔。','Choose a note to revisit, or start a new one.')} action={<Button onClick={add}>{t('写一条随笔','Write a note')}</Button>}/>}</Card>
    </SplitPane>}
    <Dialog open={remove} onClose={() => { if (!deleting) setRemove(false) }} aria-label={t('删除随笔','Delete note')}><h2>{t('删除这条随笔？','Delete this note?')}</h2><p>{t('删除后无法恢复。','This cannot be undone.')}</p><div className="markpad-actions"><Button disabled={deleting} onClick={() => setRemove(false)}>{t('取消','Cancel')}</Button><Button variant="danger" disabled={deleting} aria-busy={deleting} onClick={() => void deleteNote()}>{deleting ? t('删除中…','Deleting…') : t('删除','Delete')}</Button></div></Dialog>
  </PluginPage>
}
