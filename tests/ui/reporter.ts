...
export default class DesignReporter implements Reporter {
  ...
  onBegin(_config: FullConfig, suite: Suite) {
    ...
    this.complete = ['catppuccin-latte', 'catppuccin-mocha', 'rose-pine-dawn', 'rose-pine-moon', 'tokyo-night', 'tokyo-night-day', 'nord', 'github-light', 'dracula'].every(theme => ['900x600', '1280x800', '1600x1000'].every(size => titles.has(`${theme} ${size}`))) && ['plugin empty states', 'plugin error states', 'live theme and typography update preserves plugin document and UI state', 'shared controls retain keyboard focus and modal focus containment'].every(title => titles.has(title))
  }
