...
for (const theme of THEMES) for (const scheme of COLOR_SCHEMES) test(`${theme.id} ${scheme.id} readable semantic pairs`, async ({ page }) => {
  ...
  const pairs = await page.evaluate(() => {
    ...
    const color = (token: string) => {
      ...
      const base = document.createElement('span'); base.style.color = 'var(--dw-bg)'; root.append(base)
      ctx.clearRect(0, 0, 1, 1); ctx.fillStyle = getComputedStyle(base).color; ctx.fillRect(0, 0, 1, 1); base.remove()
      ctx.fillStyle = getComputedStyle(el).color; ctx.fillRect(0, 0, 1, 1); el.remove()
      const channels = [...ctx.getImageData(0, 0, 1, 1).data].slice(0, 3).map(x => x / 255).map(x => x <= .04045 ? x / 12.92 : ((x + .055) / 1.055) ** 2.4)
      return channels[0] * .2126 + channels[1] * .7152 + channels[2] * .0722
    }
    const cases: [string, string, number][] = []
    for (const text of ['text', 'text-muted']) for (const bg of ['bg', 'surface', 'surface-subtle', 'surface-raised', 'material-panel', 'material-card']) cases.push([text, bg, 4.5])
