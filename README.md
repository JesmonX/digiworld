...
- **主流键盘物理配列**：完美仿真并渲染 108 键、104 键、98 键（96%）、87 键（TKL）、84 键（75%）、68 键（65%）和 61 键（60%）主流物理配列。
...
使用保存于系统凭据库的 GitHub Token，选择多个可访问仓库，只读查看由当前账号触发的工作流、分支、提交与活跃 Job 状态。

### 5. Servers

通过系统 OpenSSH 监控远程 Linux 设备的内存、CPU、多挂载点磁盘、NVIDIA GPU 与网卡实时速度。可在用户明确点击后安装 vnStat，并显示今日累计及近 30 天每日流量。
