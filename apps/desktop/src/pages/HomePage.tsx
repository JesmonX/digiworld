...
import { Button, Metric, Panel } from '@digiworld/design-system/react'
import { Activity, AlertTriangle, Boxes, ChevronRight, Gauge, Library, RefreshCw, ShieldCheck } from 'lucide-react'
import { motion } from 'framer-motion'
import type { PluginSummary } from '@digiworld/plugin-sdk'
import { PluginIcon } from '../components/PluginIcon'
import { stateLabel } from '../components/PluginStatus'

function SummaryMetric({ label, value, detail, icon, tone = 'accent' }: { label: string; value: string | number; detail: string; icon: ReactNode; tone?: 'accent' | 'success' | 'warning' | 'neutral' }) {
  return <Panel padding="sm" className={`summary-card ${tone}`}><span className="summary-card-icon">{icon}</span><Metric label={label} value={value} hint={detail} /></Panel>
}

export function HomePage({ plugins, version, onCatalog, onOpen, onRefresh, reducedMotion }: { plugins: PluginSummary[]; version: string | undefined; onCatalog(): void; onOpen(id: string): void; onRefresh(): void; reducedMotion: boolean }) {
  ...
  return (
    <div className="home-dashboard">
      <div className="home-intro">
        <div>
          <span className="eyebrow"><Activity />Workspace overview</span>
          <h2>管理你的本地工具</h2>
      ...
      <div className="dashboard-summary" aria-label="Digiworld 状态摘要">
        <SummaryMetric label="已安装" value={plugins.length} detail="个功能" icon={<Boxes />} tone="accent" />
        <SummaryMetric label="运行中" value={running} detail={`共 ${plugins.length} 个`} icon={<Gauge />} tone="success" />
        <SummaryMetric label="需关注" value={attention} detail={attention ? '请查看状态' : '暂无异常'} icon={attention ? <AlertTriangle /> : <ShieldCheck />} tone={attention ? 'warning' : 'success'} />
        <SummaryMetric label="当前版本" value={version ?? '—'} detail="Digiworld" icon={<ShieldCheck />} tone="neutral" />
      </div>
