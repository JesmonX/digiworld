...
import { Button, Card, Panel } from '@digiworld/design-system/react'
  ...
  return (
    <div className={`summary-card ${tone}`}>
      ...
      <div className="summary-card-text">
        ...
        <div className="summary-card-val-wrap">
          <strong className="summary-card-value dw-numeric">{value}</strong>
    ...
    <div className="home-dashboard">
      <div className="home-hero-status">
        <div className="home-hero-copy">
          ...
          <p className="home-hero-desc">{healthy ? '你的本地数字工作台 · 工具运行正常' : `${attention} 个功能需要你的注意`}</p>
        </div>
        <div className={`health-pill ${healthy ? 'healthy' : 'attention'}`}>
          <span className="health-dot" />
          <span>{healthy ? '运行稳定' : '需要关注'}</span>
        </div>
      </div>

      {/* Metric Strip: Compact 64-72px items */}
      ...
      <div className="section-heading installed-heading">
        <div>
          <span className="section-kicker">你的工作台</span>
          <h2>已安装功能</h2>
        </div>
        <Button className="secondary compact" onClick={onCatalog}>
          <Library />添加功能
        </Button>
      </div>

      {/* 2-column layout for installed cards */}
      <div className="installed-list">
        {plugins.map((plugin, index) => (
          <motion.button
            ...
            transition={reducedMotion ? { duration: 0 } : { delay: index * .03, duration: .16 }}
          >
            ...
            <div className="plugin-row-copy">
              ...
              <div className="plugin-row-meta">
                ...
                {plugin.uiDesignVersion !== 1 && (
                  <small className="legacy-design">浅色兼容 · 待适配新外观</small>
                )}
              </div>
            </div>
          </motion.button>
      ...
      {/* Quick Actions (bottom soft actions) */}
