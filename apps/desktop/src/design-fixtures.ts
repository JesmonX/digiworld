...
export function fixture(method: string, payload: unknown = {}): unknown {
  ...
  if (method === 'servers.settings.get') return { devices: empty ? [] : [{ id: 'gpu1', label: 'GPU Server', host: 'gpu1', disks: ['/'], interfaces: ['eth0'], showCpu: true, showGpu: true, showTraffic: true }] }
  if (method === 'servers.sample') return { devices: empty ? [] : [{ id: 'gpu1', label: 'GPU Server', hostname: 'compute-01', timestamp: 1788580800, uptimeSeconds: 864000, memory: { total: 68719476736, used: 34359738368 }, cpu: { logicalCores: 32, load1: 8.2, load5: 7.4 }, disks: [{ device: '/dev/nvme0n1p2', mount: '/', total: 1099511627776, used: 549755813888, percent: 50 }], gpus: [{ index: 0, name: 'NVIDIA L40', utilization: 72, memoryUsedMiB: 30000, memoryTotalMiB: 46068, temperatureC: 61 }], network: [{ name: 'eth0', receivedBytes: 42949672960, sentBytes: 10737418240 }], vnstat: {}, selection: { disks: ['/'], interfaces: ['eth0'], showCpu: true, showGpu: true, showTraffic: true } }] }
  ...
  if (method === 'calendar.cached' || method === 'calendar.sync') return { calendars: [{ id: '/demo/calendar/', name: '个人', href: 'https://caldav.icloud.com/demo/calendar/', readOnly: false }], events: empty ? [] : [{ id: 'event-1', calendarId: '/demo/calendar/', href: 'https://caldav.icloud.com/demo/calendar/event.ics', etag: '1', title: '产品评审', start: '20260905T090000Z', end: '20260905T100000Z', allDay: false, location: '线上', notes: '', recurring: false }] }
