...
const account = { id: 'demo', provider: 'custom', label: '工作邮箱', email: 'hello@example.com', username: 'hello@example.com', host: 'imap.example.com', port: 993, useProxy: true, hasCredential: true, syncPhase: 'idle', indexed: 30, total: 30, baselineComplete: true, bodyTotal: 1799, bodyCompleted: 1799, bodyAttempted: 0, bodyFailed: 0, bodyPending: 0, lastError: null, nextSyncAt: '2026-09-05T06:00:00Z' }
...
export function fixture(method: string, payload: unknown = {}): unknown {
  ...
  if (method === 'usage.getSettings') return { localAgents: ['codex', 'claude', 'pi', 'zcode', 'agy'], localRoots: {}, sshSources: [], autoRefreshIntervalSeconds: 300, codexQuota: { sourceId: 'local', shellPreset: 'auto', preCommand: '', refreshIntervalSeconds: null }, agyQuota: { sourceId: 'local', shellPreset: 'auto', preCommand: '', refreshIntervalSeconds: 60 } }
  ...
  if (method === 'usage.getCodexQuota') return { status: 'ready', sourceId: 'local', sourceLabel: '本机', fetchedAt: '2026-09-05T04:00:00Z', planType: 'Plus', windows: [{ usedPercent: 32, windowDurationMins: 300, resetsAt: null }, { usedPercent: 62, windowDurationMins: 10080, resetsAt: null }], credits: { balance: '$12.50', hasCredits: true, unlimited: false }, resetCredits: { availableCount: 2, credits: [{ id: 'weekly-reset', title: 'Full reset (Weekly + 5 hr)', grantedAt: 1788500000, expiresAt: 1789500000, status: 'available' }, { id: 'long-reset', title: 'Extended reset with a deliberately long validity label', grantedAt: 1788500000, expiresAt: 1789800000, status: 'available' }] } }
  if (method === 'usage.getAgyQuota') return { status: 'ready', sourceId: 'local', sourceLabel: '本机', fetchedAt: '2026-09-05T04:00:00Z', planType: 'Pro tier', description: 'Gemini models and Claude/GPT models have separate quotas.', groups: [{ name: 'Gemini Models', description: 'Gemini 2.5 Pro & Flash', buckets: [{ id: 'gemini-5h', name: 'Gemini 5h Limit', window: '5h', windowDurationMins: 300, usedPercent: 15, remainingPercent: 85, remainingFraction: 0.85, resetTime: '2026-09-05T09:00:00Z', resetsAt: 1788598800 }, { id: 'gemini-weekly', name: 'Gemini Weekly Limit', window: 'weekly', windowDurationMins: 10080, usedPercent: 63, remainingPercent: 37, remainingFraction: 0.37, resetTime: '2026-09-12T00:00:00Z', resetsAt: 1789171200 }] }, { name: 'Claude and GPT models', description: 'Claude and GPT models share a weekly quota.', buckets: [{ id: 'claude-5h', name: 'Claude and GPT 5 hour limit', window: '5h', windowDurationMins: 300, usedPercent: 28, remainingPercent: 72, remainingFraction: 0.72, resetTime: '2026-09-05T09:00:00Z', resetsAt: 1788598800 }, { id: 'claude-weekly', name: 'Claude and GPT Weekly Limit', window: 'weekly', windowDurationMins: 10080, usedPercent: 50, remainingPercent: 50, remainingFraction: 0.5, resetTime: '2026-09-12T00:00:00Z', resetsAt: 1789171200 }] }], windows: [{ id: 'gemini-5h', name: 'Gemini 5h Limit', window: '5h', windowDurationMins: 300, usedPercent: 15, remainingPercent: 85, remainingFraction: 0.85, resetTime: '2026-09-05T09:00:00Z', resetsAt: 1788598800 }, { id: 'gemini-weekly', name: 'Gemini Weekly Limit', window: 'weekly', windowDurationMins: 10080, usedPercent: 63, remainingPercent: 37, remainingFraction: 0.37, resetTime: '2026-09-12T00:00:00Z', resetsAt: 1789171200 }], error: null }
  ...
  if (method === 'git.runs.snapshot') return {
    ...
    runs: empty ? [] : [
      ...
      { id: 2, repository: 'JesmonX/digiworld', name: 'CI', title: 'Previous CI', branch: 'main', sha: 'aabbccddeeff', status: 'completed', conclusion: 'success', url: 'https://github.com', createdAt: '2026-09-04T04:00:00Z', jobsLoaded: true, jobs: [{ id: 3, name: 'Linux test', status: 'completed', conclusion: 'success', steps: [{ name: 'Checkout', status: 'completed', conclusion: 'success', number: 1 }, { name: 'Report upload', status: 'in_progress', conclusion: null, number: 2 }] }] },
    ],
  }
  ...
  if (method === 'calendar.cached' || method === 'calendar.sync') return {
    calendars: [{ id: '/demo/calendar/', name: '个人', href: 'https://caldav.icloud.com/demo/calendar/', readOnly: false, capabilities: permissions ? { create: 'unknown', update: 'unknown', delete: 'unknown' } : { create: 'allowed', update: 'allowed', delete: 'allowed' } }],
    events: empty ? [] : [
