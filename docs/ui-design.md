...
The host uses a fixed 72px navigation rail on desktop (64px at narrow widths), an integrated window-control strip, and a transparent plugin canvas. The ambient gradient belongs to the host canvas; ordinary cards remain neutral material surfaces. Glass blur is limited to structural panels, dialogs and menus when explicitly enabled. Plugin documents must keep `html`, `body` and `#root` transparent so the host surface remains continuous.
